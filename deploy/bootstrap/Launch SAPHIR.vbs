...
Dim expectedCanonicalServerPath
Dim expectedLegacyServerPath
...
activeReleaseId = ""
metadataServerPath = ""
expectedCanonicalServerPath = ""
expectedLegacyServerPath = ""
...
If Len(activeReleaseId) > 0 And Len(metadataServerPath) > 0 Then
    expectedCanonicalServerPath = fso.BuildPath(fso.BuildPath(fso.BuildPath(localRoot, "versions"), activeReleaseId), "app\backend\saphir-server.ps1")
    expectedLegacyServerPath = fso.BuildPath(fso.BuildPath(fso.BuildPath(localRoot, "versions"), activeReleaseId), "apps\admin\backend\admin-server.ps1")
    expectedCanonicalServerPath = fso.GetAbsolutePathName(expectedCanonicalServerPath)
    expectedLegacyServerPath = fso.GetAbsolutePathName(expectedLegacyServerPath)
    metadataServerPath = fso.GetAbsolutePathName(metadataServerPath)
    warmReleaseMatches = (StrComp(expectedCanonicalServerPath, metadataServerPath, vbTextCompare) = 0 Or StrComp(expectedLegacyServerPath, metadataServerPath, vbTextCompare) = 0)
End If
