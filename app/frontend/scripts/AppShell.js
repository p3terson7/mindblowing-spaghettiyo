...
const MANAGER_VIEW_SCRIPT_SOURCES = {
  ...
  projectsView: [MANAGER_SCRIPT_SOURCE.chart, MANAGER_SCRIPT_SOURCE.projects],
};
...
function setSelfGc179ProfileForm(profile, displayName) {
  ...
  document.getElementById("selfGc179GroupInput").value = normalized.group;
  document.getElementById("selfGc179SubGroupInput").value = normalized.subGroup;
...
function updateSelfGc179MappingPreview() {
  const groupInput = document.getElementById("selfGc179GroupInput");
  const subGroupInput = document.getElementById("selfGc179SubGroupInput");
  const levelInput = document.getElementById("selfGc179LevelInput");
  ...
  const levelPreview = document.getElementById("selfGc179LevelPreview");

  if (groupPreview) {
    groupPreview.textContent = normalizeGc179Group(groupInput ? groupInput.value : "");
  }
  if (subGroupPreview) {
    subGroupPreview.textContent = normalizeGc179SubGroup(subGroupInput ? subGroupInput.value : "");
  }
  if (levelPreview) {
    levelPreview.textContent = normalizeGc179Level(levelInput ? levelInput.value : "") || "—";
  }
}
...
function getSelfGc179ProfileForm(displayName) {
  return normalizeSelfGc179Profile({
    ...
    group: document.getElementById("selfGc179GroupInput").value,
    subGroup: document.getElementById("selfGc179SubGroupInput").value,
...
function resolvePreferredView(user) {
  ...
  const projectWorkspaceRoute = String(window.location && window.location.hash || "").match(/^#projects\/[^/]+$/);
  if (projectWorkspaceRoute && allowedViews.indexOf("projectsView") >= 0) {
    return "projectsView";
  }
...
async function openEmployeeEntryInPeopleView(employeeCode, projectCode = "", entryId = "") {
  const normalizedEmployeeCode = String(employeeCode || "").trim();
  if (!normalizedEmployeeCode) {
    return false;
  }

  await ensureManagerAssetsForView("employeesView");
  if (typeof window.openPeopleProjectFilter !== "function") {
    throw new Error(t("employees.loadError"));
  }

  await window.openPeopleProjectFilter(
...
window.openEmployeeEntryInPeopleView = openEmployeeEntryInPeopleView;

async function openProjectInReview(projectCode) {
  const normalizedProjectCode = String(projectCode || "").trim();
  if (!normalizedProjectCode) {
    return false;
  }

  await ensureManagerAssetsForView("adminView");
  if (typeof showView === "function") {
    showView("adminView");
  }
  const refreshed = await refreshViewById("adminView", { force: true });
  if (refreshed === false) {
    throw new Error(t("review.loadError"));
  }

  const projectFilter = document.getElementById("reviewProjectFilter");
  if (!projectFilter || !Array.from(projectFilter.options).some(option => option.value === normalizedProjectCode)) {
    throw new Error(t("projects.projectUnavailableInReview"));
  }
  const approvalsSearch = document.getElementById("approvalsSearchInput");
  const employeeFilter = document.getElementById("reviewEmployeeFilter");
  const startDateFilter = document.getElementById("reviewStartDate");
  const endDateFilter = document.getElementById("reviewEndDate");
  if (approvalsSearch) {
    approvalsSearch.value = "";
  }
  if (employeeFilter) {
  ...
  projectFilter.value = normalizedProjectCode;
  ...
  if (pendingTab) {
    if (window.bootstrap && bootstrap.Tab && typeof bootstrap.Tab.getOrCreateInstance === "function") {
      bootstrap.Tab.getOrCreateInstance(pendingTab).show();
    } else {
      pendingTab.click();
    }
  }
  projectFilter.focus({ preventScroll: true });
  return true;
}

async function openProjectEntriesInPeople(projectCode) {
  const normalizedProjectCode = String(projectCode || "").trim();
  if (!normalizedProjectCode) {
    return false;
  }
  ...
  const refreshed = await refreshViewById("employeesView");
  if (refreshed === false) {
    throw new Error(t("employees.loadError"));
  }

  const projectFilter = document.getElementById("employeesProjectSelect");
  if (!projectFilter || !Array.from(projectFilter.options).some(option => option.value === normalizedProjectCode)) {
    throw new Error(t("projects.projectUnavailableInPeople"));
  }
  const employeeSearch = document.getElementById("employeesSearchInput");
  const employeeScope = document.getElementById("employeesScopeSelect");
  if (employeeSearch) {
    employeeSearch.value = "";
  }
  if (employeeScope) {
    employeeScope.value = "all";
    employeeScope.dispatchEvent(new Event("change", { bubbles: true }));
  }
  projectFilter.value = normalizedProjectCode;
  projectFilter.dispatchEvent(new Event("change", { bubbles: true }));
  projectFilter.focus({ preventScroll: true });
...
window.openProjectInReview = openProjectInReview;
window.openProjectEntriesInPeople = openProjectEntriesInPeople;
