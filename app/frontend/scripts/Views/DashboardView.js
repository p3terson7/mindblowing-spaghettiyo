...
function resolveDashboardEmployeeSearchValue(value) {
  ...
  const tokens = window.Saphir.textSearch.tokenize(normalizedValue);
  if (tokens.length === 0) {
    return null;
  }
  ...
  const matches = dashboardState.employees.filter(employee => {
    ...
    return window.Saphir.textSearch.matchesAll(text, tokens);
  });
...
function setDashboardSelectedEmployee(employeeCode, options = {}) {
  ...
  if (options.syncSearchInput !== false) {
...
function clearDashboardSelectedEmployeeForSearchInput(options = {}) {
  const employeeSelect = document.getElementById("employeeSelect");
  if (!employeeSelect || !employeeSelect.value) {
    return false;
  }

  setDashboardSelectedEmployee("", {
...
function resolveDashboardEmployeeSearchInput(options = {}) {
  ...
  if (!employee) {
    ...
    clearDashboardSelectedEmployeeForSearchInput();
    return false;
  }
...
function scheduleDashboardEmployeeSearchResolve() {
  const searchInput = document.getElementById("dashboardEmployeeSearchInput");
  const employeeSelect = document.getElementById("employeeSelect");
  const selectedEmployee = employeeSelect ? getDashboardEmployeeByCodeValue(employeeSelect.value) : null;
  if (searchInput && selectedEmployee) {
    const normalizedValue = String(searchInput.value || "").trim().toLowerCase();
    const selectedValues = [
      selectedEmployee.code,
      selectedEmployee.name,
      getDashboardEmployeeSearchLabel(selectedEmployee),
...
function getDashboardProjectByCode(projectCode) {
  return findProjectByCode(dashboardState.projects, projectCode);
}

function renderDashboardEntryProjectIdentity(entry) {
  if (isDiverseEntry(entry)) {
    return `<span class="inline-code-pill">${escapeHtml(t("shared.diverse"))}</span>`;
  }
  const projectCode = String(entry && entry.projectCode || "").trim();
  if (!projectCode) {
    return `<span class="inline-code-pill">${escapeHtml(t("shared.noProject"))}</span>`;
  }
  return renderProjectIdentityPill(getDashboardProjectByCode(projectCode) || projectCode, projectCode);
}
  ...
  container.innerHTML = responsibleProjects.map(item => {
    ...
    return `
      <div class="project-scope-pill">
        ${renderProjectIdentityPill(project, projectCode)}
...
function renderDashboardRecentActivity(entries) {
  ...
  const searchTokens = window.Saphir.textSearch.tokenize(searchTerm);
  const filteredEntries = searchTerm
    ? recentEntries.filter(entry => {
      const combinedText = getHistorySearchText(entry);
      return window.Saphir.textSearch.matchesAll(combinedText, searchTokens);
    })
    : recentEntries;
              ...
              <tr class="dashboard-entry-row${statusTone === "pending" ? " is-pending" : ""}${isOpen ? " is-open" : ""}">
                ...
                <td class="dashboard-entry-col-note">
                  <div class="dashboard-entry-note-tools">
                    ${renderEntryNotesPreview(entry, { showEmpty: true })}
                    ${canModify ? `
                      <button type="button" class="dashboard-note-edit-button" data-date="${escapeHtml(entry.date)}" data-punchin="${escapeHtml(entry.punchIn)}" data-message="${escapeHtml(entry.message || "")}" aria-label="${escapeHtml(t("action.editSupervisorNote"))}" title="${escapeHtml(t("action.editSupervisorNote"))}">
...
async function refreshDashboardView() {
  try {
    ...
    const detailLoaded = await fetchEmployeeData();
...
async function refreshAfterEntryMutation(employeeCode, refreshPeopleEmployeeCode = "") {
  ...
  if (activeViewId === "adminView" && typeof window.requestAppViewRefresh === "function") {
    const previousReviewScrollTop = window.scrollY;
    await window.requestAppViewRefresh(affectedViews, { forceActive: true });
    window.scrollTo({ top: previousReviewScrollTop, behavior: "auto" });
    return;
  }
...
async function deleteEntry(button, employeeCodeOverride = "") {
  const employeeCode = String(employeeCodeOverride || document.getElementById("employeeSelect").value || "").trim();
  const date = button.getAttribute("data-date");
...
async function openEmployeeFileFromDashboard(employeeCode, projectCode = "", entryId = "") {
  ...
  if (typeof window.openPeopleProjectFilter === "function") {
    await window.openPeopleProjectFilter(normalizedEmployeeCode, String(projectCode || "").trim(), String(entryId || "").trim());
    return;
  }
...
document.getElementById("punchClockEntries").addEventListener("click", event => {
  const noteButton = event.target.closest(".dashboard-note-edit-button");
  if (noteButton) {
