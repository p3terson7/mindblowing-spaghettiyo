...
function clearProjectWorkspaceRouteWhenLeaving(viewId) {
  if (viewId === "projectsView" || !/^#projects\/[^/]+$/.test(String(window.location && window.location.hash || ""))) {
    return;
  }

  if (typeof window.closeProjectWorkspaceForViewChange === "function") {
    window.closeProjectWorkspaceForViewChange();
  }
...
function showView(viewId) {
  ...
  clearProjectWorkspaceRouteWhenLeaving(resolvedViewId);
...
window.addEventListener("load", () => {
  const savedView = /^#projects\/[^/]+$/.test(String(window.location && window.location.hash || ""))
    ? "projectsView"
    : (localStorage.getItem("activeView") || "dashboardView");
  showView(savedView);
});
