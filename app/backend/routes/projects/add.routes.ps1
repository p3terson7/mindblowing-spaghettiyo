        ...
        if ($request.HttpMethod -eq "POST" -and $request.Url.AbsolutePath -eq "/projects") {
            ...
            if (-not [string]::IsNullOrWhiteSpace($requestedColorKey) -and -not (Test-ProjectColorKey -ColorKey $requestedColorKey)) {
            ...
            try {
                ...
                if ($projectMutationCommitted) {
                    $cacheWarning = Invoke-PostCommitActionSafely -Description "Project saved, but local cache invalidation failed" -Action {
