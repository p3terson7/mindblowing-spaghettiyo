        ...
        if ($request.HttpMethod -eq "DELETE" -and $request.Url.AbsolutePath -match "^/projects/([^/]+)$") {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        if ($permanentDelete) {
                            ...
                            $wasAlreadyArchived = Test-ProjectArchived -Project $projects[$projectIndex]
                            if (-not $wasAlreadyArchived) {
                                $projects[$projectIndex].archived = $true
                            }
                        }

                        if ($permanentDelete -or -not $wasAlreadyArchived) {
                ...
                if ($projectMutationCommitted -or (-not $permanentDelete -and $wasAlreadyArchived)) {
                    $cacheWarning = Invoke-PostCommitActionSafely -Description "Project removal saved, but local cache invalidation failed" -Action {
            ...
            $successMessage = if ($permanentDelete) {
                "Project deleted permanently."
            }
            elseif ($wasAlreadyArchived) {
                "Project was already archived."
            }
            else {
            ...
            respondWithSuccess $response (([PSCustomObject]@{
                ...
                alreadyArchived = if ($permanentDelete) { $false } else { $wasAlreadyArchived }
