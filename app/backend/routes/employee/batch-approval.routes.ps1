                ...
                foreach ($employeeCode in $groupedEntries.Keys) {
                    ...
                    if ($sharedDataUnavailable) {
                        foreach ($preparedEntry in $employeeRequests) {
                            & $addBatchFailure $preparedEntry "shared_data_unavailable" "The shared data folder became unavailable. This entry was not processed."
                        }
                        continue
                    }
                    ...
                    try {
                        ...
                        $isSharedDataUnavailable = ($null -ne $_.Exception.Data -and
                            $_.Exception.Data.Contains("SaphirHttpStatusCode") -and
                            [int]$_.Exception.Data["SaphirHttpStatusCode"] -eq 503)
                        if ($isSharedDataUnavailable -and $updatedCount -eq 0) {
                            throw
                        }
                        if ($isSharedDataUnavailable) {
                        ...
                        foreach ($preparedEntry in $employeeRequests) {
                            $reasonCode = if ($isSharedDataUnavailable) { "shared_data_unavailable" } else { "employee_processing_failed" }
                            $failureMessage = if ($isSharedDataUnavailable) {
                                "The shared data folder became unavailable. This entry was not processed."
                            }
                            else {
                                "The employee's entries could not be processed safely."
                            }
                            & $addBatchFailure $preparedEntry $reasonCode $failureMessage
                        }
                        continue
                    }
                    ...
                    try {
                        ...
                        foreach ($preparedEntry in $employeeRequests) {
                            $reasonCode = if ($isSharedDataUnavailable) { "shared_data_unavailable" } else { "employee_processing_failed" }
                            $failureMessage = if ($isSharedDataUnavailable) {
                                "The shared data folder became unavailable. Refresh before retrying this entry."
                            }
                            else {
                                "The employee's entries could not be processed safely."
                            }
