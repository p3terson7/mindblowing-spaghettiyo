$gc179ProfileModuleManifest = Join-Path -Path $PSScriptRoot -ChildPath "../modules/Saphir.Gc179Profile.psd1"
Import-Module -Name $gc179ProfileModuleManifest -Force -ErrorAction Stop | Out-Null
Remove-Variable -Name gc179ProfileModuleManifest -ErrorAction SilentlyContinue

$userAccessProfileModuleManifest = Join-Path -Path $PSScriptRoot -ChildPath "../modules/Saphir.UserAccessProfile.psd1"
Import-Module -Name $userAccessProfileModuleManifest -Force -ErrorAction Stop | Out-Null
Remove-Variable -Name userAccessProfileModuleManifest -ErrorAction SilentlyContinue
...
function Get-NormalizedRoleName {
    ...
    return (Saphir.UserAccessProfile\Get-NormalizedRoleName -Role $Role)
}
...
function ConvertTo-Gc179UpperText {
    ...
    return (Saphir.Gc179Profile\ConvertTo-Gc179UpperText -Value $Value)
}

function Get-Gc179NamePartsFromDisplayName {
    param([AllowNull()][string]$DisplayName)

    return (Saphir.Gc179Profile\Get-Gc179NamePartsFromDisplayName -DisplayName $DisplayName)
}
...
function ConvertTo-Gc179BooleanValue {
    ...
    return (Saphir.Gc179Profile\ConvertTo-Gc179BooleanValue -Value $Value -DefaultValue $DefaultValue)
}

function ConvertTo-Gc179PriText {
    param([AllowNull()][string]$Value)

    return (Saphir.Gc179Profile\ConvertTo-Gc179PriText -Value $Value)
}
...
function ConvertTo-Gc179PositionText {
    ...
    return (Saphir.Gc179Profile\ConvertTo-Gc179PositionText -Value $Value)
}

function ConvertTo-Gc179EchelonText {
    param([AllowNull()][string]$Value)

    return (Saphir.Gc179Profile\ConvertTo-Gc179EchelonText -Value $Value)
}
...
function ConvertTo-TimeEntryTypeArray {
    ...
    return @(Saphir.UserAccessProfile\ConvertTo-TimeEntryTypeArray -Value $Value)
}

function Get-EmployeeTimeEntryTypesFromUserRecord {
    param($UserRecord)

    return @(Saphir.UserAccessProfile\Get-EmployeeTimeEntryTypesFromUserRecord -UserRecord $UserRecord)
}
...
function Get-ProjectsForCurrentUser {
    ...
    return @(Select-ProjectsByArchiveScope -Projects $projects -Scope $Scope)
}
