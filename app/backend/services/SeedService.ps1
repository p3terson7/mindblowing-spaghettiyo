...
function New-DemoOvertimeEntries {
    ...
    if ($entryCount -gt 0) {
        ...
        [void](Invoke-PostCommitActionSafely -Description "Demo entries saved, but cross-machine refresh publication failed" -Action {
            Publish-DataChange -Category "seed" -Resource $batchId | Out-Null
        })
    }
