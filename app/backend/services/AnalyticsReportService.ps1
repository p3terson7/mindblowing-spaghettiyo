...
<head>
  ...
  <style>
    :root{color-scheme:light;--bg:#fff;--panel:#fff;--panel-solid:#fff;--text:#172033;--muted:#607080;--line:#d9e1ea;--line-strong:#bdc9d7;--accent:#0868d7;--accent-hover:#0758b6;--accent-soft:#eaf3ff;--focus:rgba(8,104,215,.2);--shadow:0 4px 16px rgba(30,55,85,.07);--blue:#0868d7;--green:#16865a;--violet:#7558d8;--teal:#008994;--amber:#b56f00;--coral:#c43840;--pink:#b9477f;--indigo:#4f66c8;--graphite:#667085;--mint:#0f8f7a;--pending:#a96200;--rejected:#c43840}
    *{box-sizing:border-box}
    body{margin:0;background:var(--bg);color:var(--text);font:15px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    ...
    .bar-list{display:grid;gap:12px;max-height:600px;overflow:auto;padding-right:5px}.bar-row{display:grid;grid-template-columns:minmax(140px,220px) minmax(180px,1fr) 78px;gap:12px;align-items:center}.bar-label{display:flex;align-items:center;gap:8px;min-width:0;overflow:hidden;white-space:nowrap;font-weight:650}.bar-label-text{overflow:hidden;text-overflow:ellipsis}.project-marker{display:inline-block;flex:0 0 10px;width:10px;height:10px;background:var(--accent)}.project-marker.marker-circle{border-radius:50%}.project-marker.marker-square{border-radius:2px}.project-marker.marker-diamond{border-radius:1px;transform:rotate(45deg) scale(.8)}.project-marker.marker-triangle{clip-path:polygon(50% 0,100% 100%,0 100%)}.bar-track{height:16px;background:#edf2f7;border-radius:999px;overflow:hidden;display:flex}.bar-segment{height:100%;min-width:0;background-size:8px 8px}.bar-segment:focus-visible{outline:2px solid #fff;outline-offset:-3px}.bar-segment.marker-circle{background-image:radial-gradient(circle at 4px 4px,rgba(255,255,255,.62) 0 1.4px,transparent 1.6px)}.bar-segment.marker-square{background-image:linear-gradient(90deg,transparent 0 5px,rgba(255,255,255,.5) 5px 7px,transparent 7px)}.bar-segment.marker-diamond{background-image:repeating-linear-gradient(135deg,transparent 0 4px,rgba(255,255,255,.5) 4px 6px,transparent 6px 10px)}.bar-segment.marker-triangle{background-image:repeating-linear-gradient(45deg,transparent 0 3px,rgba(255,255,255,.52) 3px 5px,transparent 5px 9px)}.bar-value{text-align:right;font-variant-numeric:tabular-nums;font-weight:700}.empty{padding:28px;text-align:center;color:var(--muted);background:#fafbfd;border:1px dashed var(--line-strong);border-radius:11px}
...
<body>
  ...
  (()=>{"use strict";
    ...
    function projectMarkerKey(project){return["circle","square","diamond","triangle"].includes(project.markerKey)?project.markerKey:"circle"}
    function projectMarker(project){const marker=document.createElement("span");marker.className=`project-marker marker-${projectMarkerKey(project)}`;marker.style.background=colors[project.colorKey]||"var(--accent)";marker.setAttribute("aria-hidden","true");return marker}
    ...
    function renderBars(container,facts,kind){
      ...
      if(!rows.length){empty(container);return}const max=Math.max(1,...rows.map(row=>row.total));rows.forEach(row=>{const wrapper=document.createElement("div");wrapper.className="bar-row";const name=document.createElement("div"),nameText=document.createElement("span");name.className="bar-label";nameText.className="bar-label-text";nameText.textContent=label(row.item);name.title=label(row.item);if(kind==="project")name.appendChild(projectMarker(row.item));name.appendChild(nameText);const track=document.createElement("div");track.className="bar-track";
        if(kind==="employee"){const segments=grouped(facts.filter(item=>item.employeeRef===row.key),item=>item.projectCode);[...segments.entries()].sort((a,b)=>b[1]-a[1]).forEach(([projectCode,value])=>{const project=projects.get(projectCode)||{},segment=document.createElement("span"),description=`${project.displayName||projectCode}: ${formatHours(value)}`;segment.className=`bar-segment marker-${projectMarkerKey(project)}`;segment.style.width=`${value/max*100}%`;segment.style.backgroundColor=colors[project.colorKey]||"var(--accent)";segment.title=description;segment.tabIndex=0;segment.setAttribute("role","img");segment.setAttribute("aria-label",description);track.appendChild(segment)})}else{const segment=document.createElement("span"),description=`${label(row.item)}: ${formatHours(row.total)}`;segment.className=`bar-segment marker-${projectMarkerKey(row.item)}`;segment.style.width=`${row.total/max*100}%`;segment.style.backgroundColor=colors[row.item.colorKey]||"var(--accent)";segment.title=description;segment.tabIndex=0;segment.setAttribute("role","img");segment.setAttribute("aria-label",description);track.appendChild(segment)}
