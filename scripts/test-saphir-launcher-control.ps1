...
$serverScript = Join-Path -Path $sourceRoot -ChildPath "apps/admin/backend/admin-server.ps1"
...
try {
    ...
    Set-Content `
        -LiteralPath (Join-Path -Path (Split-Path -Path $serverScript -Parent) -ChildPath "admin-config.psd1") `
    ...
    $cachedServerScript = Join-Path -Path $cachedReleaseRoot -ChildPath "apps/admin/backend/admin-server.ps1"
