...
function createAdminPollHarness() {
  const source = readSource("apps/admin/frontend/scripts/AppShell.js");
  const state = {
    syncRequestInFlight: false,
    lastSyncVersion: 5,
    lastSyncChangeKey: "id:initial",
  };
  const currentUser = { username: "manager", role: "admin" };
  const calls = {
    handledStates: [],
    targetedStates: [],
    targetedStaleViews: [],
...
function createEmployeePollHarness() {
  const source = readSource("apps/employee/frontend/scripts/EmployeeShell.js");
  const state = {
    lastSyncVersion: 30,
  ...
  const context = {
    ...
    getEmployeeSessionToken: () => "token",
    fetch: async () => ({}),
    parseResponse: async () => syncState,
    async refreshEmployeeView() {
      calls.refreshes += 1;
      if (failNextRefresh) {
        failNextRefresh = false;
        throw new Error("synthetic employee refresh failure");
      }
    },
  };
  ...
  vm.createContext(context);
  vm.runInContext([
    getFunctionSource(source, "getEmployeeSyncStateChangeKey", "function normalizeEmployeeApiUrl"),
    getFunctionSource(source, "pollEmployeeSyncState", "function startEmployeeSyncPolling"),
    "this.poll = pollEmployeeSyncState;",
  ].join("\n"), context);
...
const changeKeys = [
  loadChangeKey("apps/admin/frontend/scripts/AppShell.js", "getSyncStateChangeKey", "function getStoredTheme"),
  loadChangeKey(
    "apps/employee/frontend/scripts/EmployeeShell.js",
    "getEmployeeSyncStateChangeKey",
    "function normalizeEmployeeApiUrl"
  ),
];
...
async function testAdminGapRouting() {
  const harness = createAdminPollHarness();
...
async function testEmployeeRefreshRetry() {
  const harness = createEmployeePollHarness();
  harness.setSyncState({
    version: 31,
  ...
  await harness.poll();
  assert.equal(harness.state.lastSyncVersion, 30, "A failed employee refresh must retain the previous revision");
  assert.equal(harness.state.lastSyncChangeKey, "id:employee-initial");

  await harness.poll();
  assert.equal(harness.calls.refreshes, 2, "The employee client must retry an unchanged event after refresh failure");
  assert.equal(harness.state.lastSyncVersion, 31);
...
Promise.resolve()
  ...
  .then(testEmployeeRefreshRetry)
  .then(() => {
    console.log("Client sync test passed: gap routing is conservative and failed refreshes retry.");
  })
  .catch(error => {
    console.error(error);
    process.exitCode = 1;
