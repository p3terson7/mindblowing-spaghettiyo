...
try {
    ...
    Ensure-TestDirectory -Path (Join-Path -Path $releasePath -ChildPath "apps/admin/backend")
    ...
    Set-Content -LiteralPath (Join-Path -Path $releasePath -ChildPath "apps/admin/backend/admin-server.ps1") -Value "# fixture" -Encoding UTF8
