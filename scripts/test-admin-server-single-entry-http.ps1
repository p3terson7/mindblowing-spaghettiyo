...
$repoRoot = (Get-Item -LiteralPath (Join-Path -Path $scriptRoot -ChildPath "..")).FullName
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path -Path $repoRoot -ChildPath "apps") -Destination $runtimeRoot -Recurse -Force
    $repositoryV1Data = Join-Path -Path $repoRoot -ChildPath "v1_data/data"
    Assert-True -Condition (Test-Path -LiteralPath $repositoryV1Data -PathType Container) -Message "The repository v1 data fixture is missing."
    Copy-Item -Path (Join-Path -Path $repositoryV1Data -ChildPath "*") -Destination $dataRoot -Recurse -Force
...
"@
    $configPath = Join-Path -Path $runtimeRoot -ChildPath "apps/admin/backend/admin-config.psd1"
    ...
    Assert-True -Condition $legacyEntryText.TrimStart().StartsWith("{") -Message "The v1 fixture no longer reproduces singleton-object storage."
    $legacyEntry = $legacyEntryText | ConvertFrom-Json -ErrorAction Stop
    $entryId = [string]$legacyEntry.entryId
    Assert-True -Condition (-not [string]::IsNullOrWhiteSpace($entryId)) -Message "The v1 singleton entry does not have a stable ID."
    ...
    $serverPath = Join-Path -Path $runtimeRoot -ChildPath "apps/admin/backend/admin-server.ps1"
    ...
    $serverProcess = Start-Process -FilePath $pwshPath `
    ...
    Assert-True -Condition (Test-Path -LiteralPath $schemaPath -PathType Leaf) -Message "The server did not adopt the legacy data folder into the schema contract."
