...
$routePath = Join-Path -Path $repoRoot -ChildPath "apps/admin/backend/routes/auth.routes.ps1"
...
foreach ($backendScript in @(Get-ChildItem -LiteralPath (Join-Path -Path $repoRoot -ChildPath "apps/admin/backend") -Filter "*.ps1" -File -Recurse)) {
    $lineNumber = 0
