...
for (const asset of [
  ...
  assert(index.includes(`${asset}?v=20260924-bug-report-minimal-v3`), `${asset} cache key was not bumped.`);
}
for (const view of ["EmployeesView", "DashboardView", "ApprovalsView", "ProjectsView"]) {
  assert(shell.includes(`${view}.js?v=20260924-bug-report-minimal-v3`), `${view} cache key was not bumped.`);
}
