...
for (const asset of [
  ...
  assert(index.includes(`${asset}?v=20260921-schedule-gc179-fixes-v2`), `${asset} cache key was not bumped.`);
}
for (const view of ["EmployeesView", "DashboardView", "ApprovalsView", "ProjectsView"]) {
  assert(shell.includes(`${view}.js?v=20260921-schedule-gc179-fixes-v2`), `${view} cache key was not bumped.`);
}
