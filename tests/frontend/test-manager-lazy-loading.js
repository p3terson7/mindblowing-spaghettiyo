...
assert.deepEqual(managerViewSources, {
  ...
  projectsView: [managerScriptSource.chart, managerScriptSource.projects],
});
...
async function testProjectsLoadWithoutDashboard() {
  const harness = createLoaderHarness();
  const manager = { role: "admin", employeeCode: "100002" };

  await harness.context.lazyLoader.ensureManagerAssetsForView("projectsView", manager);
  assert.deepEqual(
    harness.requestedSources,
    [managerScriptSource.chart, managerScriptSource.projects],
    "Projects must not load Dashboard now that its shared UI primitive lives in Utilities.",
  );
}
...
async function run() {
  ...
  await testProjectsLoadWithoutDashboard();
