...
function New-FixtureRelease {
    ...
    New-Item -ItemType Directory -Path (Join-Path -Path $releaseRoot -ChildPath "app/backend") -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path -Path $releaseRoot -ChildPath "app/backend/lib") -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path -Path $releaseRoot -ChildPath "app/backend/services") -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path -Path $releaseRoot -ChildPath "app/frontend") -Force | Out-Null
    ...
    Set-Content -LiteralPath (Join-Path -Path $releaseRoot -ChildPath "app/backend/saphir-server.ps1") -Value "# $ReleaseId" -Encoding UTF8
    Set-Content -LiteralPath (Join-Path -Path $releaseRoot -ChildPath "app/backend/saphir-config.psd1") -Value "@{ DataFolderPath = '$($DataFolderPath.Replace("'", "''"))' }" -Encoding UTF8
    Set-Content -LiteralPath (Join-Path -Path $releaseRoot -ChildPath "app/backend/lib/ControlService.ps1") -Value "# control service" -Encoding UTF8
    Set-Content -LiteralPath (Join-Path -Path $releaseRoot -ChildPath "app/backend/services/RouteDispatchService.ps1") -Value "# route dispatch" -Encoding UTF8
    Set-Content -LiteralPath (Join-Path -Path $releaseRoot -ChildPath "app/frontend/index.html") -Value "<html>$ReleaseId</html>" -Encoding UTF8
    ...
    Set-Content -LiteralPath (Join-Path -Path $releaseRoot -ChildPath "scripts/lib/ApplicationLayout.ps1") -Value "# application layout" -Encoding UTF8
...
try {
    ...
    Assert-True -Condition (Test-Path -LiteralPath (Join-Path -Path $warmResult.ReleasePath -ChildPath "app/backend/saphir-server.ps1") -PathType Leaf) -Message "a failed repair must preserve existing local files"
    Remove-Item -LiteralPath $releaseA.ReleasePath -Force
