...
"@
    [System.IO.File]::WriteAllText(
        (Join-Path -Path $BackendPath -ChildPath "saphir-config.psd1"),
        $content,
...
function Invoke-TestAdminContext {
    ...
    & {
        ...
        . (Join-Path -Path $FixtureBackendPath -ChildPath "lib/AppContext.ps1")
    } $BackendPath
}
...
try {
    ...
    Copy-Item `
        -LiteralPath (Join-Path -Path $repoRoot -ChildPath "app/backend/lib/AppContext.ps1") `
        -Destination (Join-Path -Path $fixtureLib -ChildPath "AppContext.ps1") `
        -Force
    Copy-Item `
        -LiteralPath (Join-Path -Path $repoRoot -ChildPath "app/backend/lib/FileStore.ps1") `
    ...
    $futureContractError = ""
    try {
        & (Join-Path -Path $repoRoot -ChildPath "tests/powershell/test-data-folder-contract.ps1") -DataFolderPath $futureFolder | Out-Null
    }
    catch {
