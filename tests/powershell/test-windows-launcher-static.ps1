...
$applicationLayoutPath = Join-Path -Path $repoRoot -ChildPath "scripts/lib/ApplicationLayout.ps1"
...
$allPowerShellSourceFiles = @(
    Get-ChildItem `
        -LiteralPath (Join-Path -Path $repoRoot -ChildPath "app"), (Join-Path -Path $repoRoot -ChildPath "scripts") `
        -Recurse `
        -File |
...
Assert-True -Condition ($interfaceSource.IndexOf('UtilityButtonStyle', [System.StringComparison]::Ordinal) -ge 0) -Message "launcher maintenance actions must use a quieter visual style"
Assert-True -Condition ($interfaceSource.IndexOf('IconButtonStyle', [System.StringComparison]::Ordinal) -ge 0) -Message "launcher secondary actions must use compact icon controls"
Assert-True -Condition ($interfaceSource.IndexOf('FontFamily="Segoe MDL2 Assets"', [System.StringComparison]::Ordinal) -ge 0) -Message "launcher action icons must use the Windows built-in symbol font"
Assert-True -Condition ($interfaceSource.IndexOf('x:Name="RuntimeActionsPanel"', [System.StringComparison]::Ordinal) -ge 0) -Message "launcher runtime controls must be grouped separately from the main action"
Assert-True -Condition ($interfaceSource.IndexOf('x:Name="OpenButton" Style="{StaticResource PrimaryButtonStyle}"', [System.StringComparison]::Ordinal) -ge 0) -Message "the current launcher action must remain visually primary"
Assert-True -Condition ($interfaceSource.IndexOf('Tag="&#xE8A7;" Width="300" HorizontalAlignment="Center"', [System.StringComparison]::Ordinal) -ge 0) -Message "the primary open action must be compact, centered, and identifiable by an icon"
Assert-True -Condition ($interfaceSource.IndexOf('Width="210"', [System.StringComparison]::Ordinal) -lt 0) -Message "launcher runtime actions must not return to oversized side-by-side buttons"
Assert-True -Condition ($interfaceSource.IndexOf('$state -eq "Online" -and -not $showUpdate', [System.StringComparison]::Ordinal) -ge 0) -Message "the launcher must show only one primary action when an update is available"
