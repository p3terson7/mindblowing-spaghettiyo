...
Assert-True -Condition ($interfaceSource.IndexOf('x:Name="OpenButton" Style="{StaticResource PrimaryButtonStyle}" HorizontalAlignment="Stretch"', [System.StringComparison]::Ordinal) -ge 0) -Message "the current launcher action must occupy one clear primary row"
