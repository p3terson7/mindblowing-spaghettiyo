...
try {
    ...
    . (Join-Path -Path $repoRoot -ChildPath "app/backend/services/DataSchemaService.ps1")
    ...
    for ($routeRun = 0; $routeRun -lt 1; $routeRun++) {
        . (Join-Path -Path $repoRoot -ChildPath "app/backend/routes/employee/batch-approval.routes.ps1")
    }
    ...
    $serverSource = [System.IO.File]::ReadAllText((Join-Path -Path $repoRoot -ChildPath "app/backend/saphir-server.ps1"))
    ...
    foreach ($contract in @(
        [PSCustomObject]@{ Path = "app/backend/routes/history.routes.ps1"; Text = "ConvertTo-Json -InputObject @(Get-HistoryEntriesSnapshot)" },
        [PSCustomObject]@{ Path = "app/backend/routes/employee/list.routes.ps1"; Text = "ConvertTo-Json -InputObject @(`$employees)" },
        [PSCustomObject]@{ Path = "app/backend/routes/projects/get.routes.ps1"; Text = "ConvertTo-Json -InputObject `$projectsData" },
        [PSCustomObject]@{ Path = "app/backend/routes/self.routes.ps1"; Text = "ConvertTo-Json -InputObject @(`$entries)" },
        [PSCustomObject]@{ Path = "app/backend/routes/stats/summary.routes.ps1"; Text = "ConvertTo-Json -InputObject `$result" }
    )) {
    ...
    foreach ($guardedRoute in @(
        "app/backend/routes/employee/add.routes.ps1",
        "app/backend/routes/employee/update.routes.ps1",
        "app/backend/routes/self.routes.ps1",
        "app/backend/routes/employee/gc179-import.routes.ps1",
        "app/backend/routes/projects/add.routes.ps1",
        "app/backend/routes/projects/restore.routes.ps1",
        "app/backend/routes/projects/update.routes.ps1",
        "app/backend/routes/projects/delete.routes.ps1"
    )) {
    ...
    foreach ($backendScript in @(Get-ChildItem -LiteralPath (Join-Path -Path $repoRoot -ChildPath "app/backend") -Recurse -File -Filter "*.ps1")) {
        $tokens = $null
        $parseErrors = $null
