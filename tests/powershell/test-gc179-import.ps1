...
$servicePath = Join-Path -Path $repoRoot -ChildPath "app/backend/services/Gc179ImportService.ps1"
$exportServicePath = Join-Path -Path $repoRoot -ChildPath "app/backend/services/Gc179ExportService.ps1"
$routePath = Join-Path -Path $repoRoot -ChildPath "app/backend/routes/employee/gc179-import.routes.ps1"
...
$generatedExamplePath = Join-Path -Path $repoRoot -ChildPath "tests/fixtures/gc179/generated-example.ps1"
...
Assert-True -Condition (Test-Path -LiteralPath $generatedExamplePath -PathType Leaf) -Message "The GC179 generation fixture is missing."
...
Assert-Equal -Expected $generatedExampleExpectedSha256 -Actual ((Get-FileHash -LiteralPath $generatedExamplePath -Algorithm SHA256).Hash.ToUpperInvariant()) -Message "The GC179 generation fixture changed; import work must preserve its exact structure."
...
$legacySingleEntryPath = Join-Path -Path $repoRoot -ChildPath "tests/fixtures/data-contract/reference-v1/000321928_data.json"
Assert-True -Condition (Test-Path -LiteralPath $legacySingleEntryPath -PathType Leaf) -Message "The tracked single-entry compatibility fixture is missing."
$legacySingleEntries = @(Read-Gc179EmployeeDataStrict -Path $legacySingleEntryPath)
Assert-Equal -Expected 1 -Actual $legacySingleEntries.Count -Message "GC179 strict reads must accept a legacy one-entry JSON object root."
