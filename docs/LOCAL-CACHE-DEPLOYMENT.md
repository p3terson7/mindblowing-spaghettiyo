...
2. Choose the existing shared GEEM data folder using its UNC path, for example `\\server\department\GEEM-Data`.
...
For the first deployment and important updates, publish first to a separate pilot parent folder and give that shortcut to one or two employees. After they confirm that sign-in, loading, saving and stopping work from the real Windows/UNC environment, run the normal production publish command above.
...
2. Test from the actual UNC network folder.
...
For local tests outside Windows/UNC storage, explicitly add `-AllowLocalDataPath`. Never use that switch for the employee distribution.
